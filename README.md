...
- 功能演进与方案覆盖关系统一记录在 [项目更新日志](项目更新日志.md)
...
传感器控制面板应用，提供硬件交互界面，并负责在主页面集中显示各传感器状态。

**主要功能**:
- **LED 控制**: 打开/关闭用户 LED
- **AP3216C 传感器**: 实时显示环境光强度和接近距离
- **DHT11 传感器**: 实时显示温度和湿度
- **SR501 人体感应**: 通过 `/dev/sr501` 的 `SIGIO` 异步通知刷新界面上现有的“有人”状态标签
...
- `/dev/sr501` - SR501 人体红外传感器设备
- `/dev/video1` - 摄像头设备 (通过 Camera_Project 访问)

**运行逻辑**:
- `AP3216C` 和 `DHT11` 当前仍通过 `QTimer` 在主线程定时刷新显示
- `SR501` 不再通过定时器轮询；应用启动后会按驱动示例执行 `F_SETOWN + O_ASYNC`，由驱动通过 `kill_fasync(SIGIO)` 触发通知
- Qt 侧使用 `socketpair + QSocketNotifier` 把 `SIGIO` 安全转进事件循环，然后再读取 `/dev/sr501` 刷新 `label_people`
- 手动“监控画面”按钮仍然保留，通过 `QProcess` 拉起 `/lib/modules/4.1.15-g3dc0a4b/Camera_Project`
- 当前 `release` 基线优先恢复“主界面显示 + 手动启动 Camera_Project”的稳定功能，多传感器自动联动和状态机回接以 [项目更新日志](项目更新日志.md) 为准，不在本段落冒充成当前已启用能力
...
| v1.2 | 2026-02-04 | **RELEASE 版本** - 集成 SR501 传感器，UI 布局优化，完善纯 Qt 渲染显示链路 |
